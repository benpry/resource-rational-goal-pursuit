...
def compute_sparsemax_log_likelihood(states, continuous_attention, goal, init_endogenous, use_exo_cost, clusters):
        ...
        pbounds = {'ac': (0., 30.), 'ss': (0., 1.5), 'exp': (0.01, 1.), 'vm': (0., 5.)}
...
def compute_sparsemax_score(continuous_attention, goal, init_endogenous, use_exo_cost, attention_cost, step_size,
    ...
    macro_agent = MicroworldMacroAgent(A=A, B=B, init_endogenous=init_endogenous,
                                       ...
                                       nr_subgoals=0, init_exogenous=torch.tensor([0, 0, 0, 0]), T=T,
                                       final_goal=goal, clamp=clamp, agent_class=agent_class,
                                       cost=attention_cost, lr=step_size, von_mises_parameter=vm_param,
                                       exponential_parameter=exp_param,
                                       continuous_attention=continuous_attention,
                                       use_exo_cost=use_exo_cost, exo_cost=exo_cost, step_with_model=add_noise,
                                       verbose=False)
...
if __name__ == '__main__':
    ...
    situation = ((torch.rand(5) - 0.5) * 500).round()
    ...
    for i in range(n_runs):
        ...
        sparse_max_states.append(generate_sparsemax_data(continuous_attention, goal, situation, use_exo_cost,
                                                         attention_cost, step_size, exp_param, vm_param))

    print(len(sparse_max_states))
    # compute the cost of the deterministic hill-climbing agent in this situation (with the generating parameters)
    sparse_max_costs = []
    for i in range(2):
        ...
        sparse_max_costs.append(compute_sparsemax_score(continuous_attention, goal, situation, use_exo_cost,
                                                        attention_cost, step_size, exp_param, vm_param, False))
    ...
    for i in range(n_runs):
        ...
        sparse_max_costs_noisy_all.append(compute_sparsemax_score(continuous_attention, goal, situation, use_exo_cost,
                                                                  attention_cost, step_size, exp_param, vm_param, True))
